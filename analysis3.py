...
OUT_DIR = BASE_DIR / "peak_timing_metric_outputs"
OUT_DIR.mkdir(parents=True, exist_ok=True)

OUT_CSV = OUT_DIR / "peak_timing_metric_summary.csv"
OUT_BOXPLOT = OUT_DIR / "peak_timing_boxplot.png"
OUT_HEATMAP = OUT_DIR / "peak_timing_heatmap.png"

# Peak timing definition
# Search the strongest pre-event peak in this window
PEAK_SEARCH_WINDOW = (-30, 0)

# Ideal lead time for prediction
# Example: peak 3 days before event is best
IDEAL_LEAD_DAYS = 3
...
def parse_pdi_name(colname: str):
    """
    Example:
        PDI_ta_R3_P7 -> temp_case='ta', rain_window=3, persist_window=7
    """
...
def calc_peak_timing_score(df: pd.DataFrame, pdi_col: str):
    """
    Peak timing metric:
      1) Find the maximum PDI timing in the pre-event search window [-30, 0]
      2) Convert to lead time: lead_days = 0 - peak_day
      3) Score = -abs(lead_days - IDEAL_LEAD_DAYS)

    Returns:
      peak_timing_score, peak_day, lead_days, peak_value
    """
    ...
    sub = work.loc[
        (work["relative_day"] >= PEAK_SEARCH_WINDOW[0]) &
        (work["relative_day"] <= PEAK_SEARCH_WINDOW[1])
    ].copy()

    sub = sub.dropna(subset=[pdi_col])
    if sub.empty:
        return np.nan, np.nan, np.nan, np.nan

    # strongest peak in pre-event window
    idx = sub[pdi_col].idxmax()
    peak_day = float(sub.loc[idx, "relative_day"])
    peak_value = float(sub.loc[idx, pdi_col])

    lead_days = 0 - peak_day
    peak_timing_score = -abs(lead_days - IDEAL_LEAD_DAYS)

    return peak_timing_score, peak_day, lead_days, peak_value
...
def main():
    ...
    for sheet_name, df in event_sheets.items():
        ...
        for col in pdi_cols:
            ...
            score, peak_day, lead_days, peak_value = calc_peak_timing_score(df, col)
    ...
    if summary.empty:
        raise RuntimeError("계산된 peak timing 결과가 없습니다.")
    ...
    for ax, temp_case, title in zip(
        ...
        ax.set_ylabel("Peak timing score")
        ax.set_title(f"{title} | Peak timing box plot")
    ...
    for ax, temp_case, title in zip(
        ...
        ax.set_title(f"{title} | Mean peak timing score", fontsize=14)
    ...
    cbar.set_label("Peak timing score", fontsize=12)

    fig.savefig(OUT_HEATMAP, dpi=220, bbox_inches="tight")
    plt.close(fig)
    print(f"[OK] saved: {OUT_HEATMAP}")

    print("[DONE] Peak timing metric analysis complete.")
