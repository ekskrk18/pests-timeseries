...
OUT_DIR = BASE_DIR / "growth_metric_outputs"
OUT_DIR.mkdir(parents=True, exist_ok=True)

OUT_CSV = OUT_DIR / "growth_metric_summary.csv"
OUT_BOXPLOT = OUT_DIR / "growth_boxplot.png"
OUT_HEATMAP = OUT_DIR / "growth_heatmap.png"

# Growth metric search range
# t in [-10, -1]
T_CANDIDATES = list(range(-10, 0))

# Recent window: [t-1, t]
RECENT_OFFSETS = (-1, 0)

# Prior window: [t-6, t-4]
PRIOR_OFFSETS = (-6, -4)

# Heatmap style
HEATMAP_CMAP = "YlOrRd"   # alternatives: "viridis", "plasma"
HEATMAP_PAD_RATIO = 0.08  # small padding around min/max
...
def max_local_growth(df: pd.DataFrame, pdi_col: str):
    """
    Growth metric:
        PGI = max_t ( mean[t-1:t] - mean[t-6:t-4] ), for t in [-10, -1]
    ...
    for t in T_CANDIDATES:
        r0, r1 = t + RECENT_OFFSETS[0], t + RECENT_OFFSETS[1]
        p0, p1 = t + PRIOR_OFFSETS[0], t + PRIOR_OFFSETS[1]

        recent_vals = work.loc[
            (work["relative_day"] >= r0) & (work["relative_day"] <= r1),
            pdi_col
        ].dropna()

        prior_vals = work.loc[
            (work["relative_day"] >= p0) & (work["relative_day"] <= p1),
            pdi_col
        ].dropna()

        if len(recent_vals) == 0 or len(prior_vals) == 0:
            continue

        growth = recent_vals.mean() - prior_vals.mean()

        if pd.isna(best_growth) or (growth > best_growth):
            best_growth = growth
            best_t = t

    return best_growth, best_t
...
def main():
    ...
    for sheet_name, df in event_sheets.items():
        ...
        for col in pdi_cols:
            ...
            growth, best_t = max_local_growth(df, col)
    ...
    if summary.empty:
        raise RuntimeError("계산된 growth 결과가 없습니다.")
    ...
    for ax, temp_case, title in zip(
        ...
        ax.set_title(f"{title} | Growth metric box plot")
    ...
    # collect actual heatmap values first
    pivots = {}
    all_heat_values = []
    ...
    pad = vrange * HEATMAP_PAD_RATIO if vrange > 0 else 0.01
    ...
    for ax, temp_case, title in zip(
        ...
        ax.set_title(f"{title} | Mean growth score", fontsize=14)
    ...
    cbar.set_label("Growth score", fontsize=12)

    fig.savefig(OUT_HEATMAP, dpi=220, bbox_inches="tight")
    plt.close(fig)
    print(f"[OK] saved: {OUT_HEATMAP}")

    print("[DONE] Growth metric analysis complete.")
